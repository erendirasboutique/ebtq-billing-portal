...
export default function CustomerPortal({ initialUser, initialPayments }) {
  ...
  const name =
    payments.find((p) => p.customer_name)?.customer_name ||
    initialUser?.email ||
    'Customer';
  ...
  async function refreshSession() {
    ...
    if (error || !data?.session?.access_token) {
      setMessage('Session not found yet. Open the email link, then come back and click this button.');
      return;
    }
        ...
        <div className="shell">
          ...
          <section className="hero">
            <div className="card loginCard">
              ...
              <h1>
                {lang === 'es'
                  ? 'Tus pagos, en un solo lugar.'
                  : 'Your payments, beautifully organized.'}
              </h1>

              <p>
                {lang === 'es'
                  ? 'Ingresa con el mismo correo que usaste para pagar.'
                  : 'Sign in with the email you used at checkout to view your lifetime payment history, receipts, and order status.'}
              </p>
              ...
              <button className="button ghost" type="button" onClick={refreshSession}>
    ...
    <main className="page">
      ...
      <div className="shell customerDashboard">
        <div className="topbar">
          ...
          <div className="lang">
            <LanguageToggle lang={lang} setLang={setLang} />
            <button className="button ghost mini" onClick={logout}>
              Logout
            </button>
          </div>
        </div>
        ...
        <section className="customerHero">
          <div>
            ...
            <p>
              {lang === 'es'
                ? 'Aquí puedes ver tus pagos, recibos y estado de pedido.'
                : 'Here is your Erendira’s Boutique payment history, receipts, and order status.'}
            </p>
          </div>
        </section>
        ...
        <div className="customerStats">
          ...
          <div className="stat">
            ...
            <b>{latest ? formatDate(latest.created_at) : '—'}</b>
          </div>
        </div>
        ...
        <section className="paymentList">
          {payments.map((p) => (
            <article
              className="orderCard customerPaymentCard"
              key={p.id || p.stripe_session_id || p.clover_payment_id}
            >
              <div className="orderTop">
                ...
                <div className="amountBlock">
                  <span className={`pill source-${p.payment_source || 'stripe'}`}>
                    {p.payment_source || 'stripe'}
                  </span>
              ...
              <div className="quickDetails">
                ...
                <div>
                  <b>Description</b>
                  <p>{p.description || 'Boutique purchase'}</p>
                </div>
                  ...
                  <p>
                    {p.tracking_url ? (
                      <a href={p.tracking_url} target="_blank" rel="noopener noreferrer">
                        {p.tracking_number || 'Track package'}
                      </a>
                    ) : (
                      p.tracking_number || 'Not shipped yet'
                    )}
                  </p>
                </div>
              </div>
          ...
          {payments.length === 0 && (
            <div className="emptyState">No payments found for this email yet.</div>
          )}
        </section>
      </div>
    </main>
